...
class support_main:
    ...
    def show_img(self):
        ...
        # if self.detect_lidar.update_vi_tri_agv == 0:
        #     img = self.detect_lidar.img2
        # else:
        #     img = self.detect_lidar.edit_img
        x_goc = self.detect_lidar.x_goc - self.detect_lidar.x_min_img2
        y_goc = self.detect_lidar.y_goc - self.detect_lidar.y_min_img2
        
        # ds_diem = self.detect_lidar.point_setup + self.detect_lidar.ds_diem
        # for i in range(0,len(ds_diem)):
        #     if i == 0:
        #         x1 = self.detect_lidar.x_goc - self.detect_lidar.x_min_img2
        #         y1 = self.detect_lidar.y_goc - self.detect_lidar.y_min_img2
        #     else:
        #         if ds_diem[i-1][0] == "diem_dich_1" or ds_diem[i-1][0] == "diem_thuong":
        #             x1 = ds_diem[i-1][1] - self.detect_lidar.x_min_img2
        #             y1 = ds_diem[i-1][2] - self.detect_lidar.y_min_img2
        #         if ds_diem[i-1][0] == "diem_dich_2":
        #             x1 = ds_diem[i-2][1] - self.detect_lidar.x_min_img2
        #             y1 = ds_diem[i-2][2] - self.detect_lidar.y_min_img2
        #     x2 = ds_diem[i][1] - self.detect_lidar.x_min_img2
        #     y2 = ds_diem[i][2] - self.detect_lidar.y_min_img2
        #     if ds_diem[i][0] == "diem_thuong":
        #         cv2.line(img,(int(x1),int(y1)),(int(x2),int(y2)),(0,255,0),1)
        #         cv2.circle(img,(int(x2),int(y2)),3,(255,0,0),-1)
        #     if ds_diem[i][0] == "diem_dich_1":
        #         cv2.line(img,(int(x1),int(y1)),(int(x2),int(y2)),(0,255,255),1)
        #         cv2.circle(img,(int(x2),int(y2)),3,(0,0,255),-1)
        #     if ds_diem[i][0] == "diem_dich_2":
        #         cv2.arrowedLine(img,(int(x1),int(y1)),(int(x2),int(y2)),(0,0,255),2,tipLength = 0.2)
        # if len(self.detect_lidar.end_point) != 0:
        #     end_point = [self.detect_lidar.end_point[0] - self.detect_lidar.x_min_img2,self.detect_lidar.end_point[1] - self.detect_lidar.y_min_img2]
        #     cv2.line(img,(int(x_goc),int(y_goc)),(int(end_point[0]),int(end_point[1])),(0,0,255),1)
        # if len(self.detect_lidar.closest_point_0) != 0:
        #     cv2.circle(img,(int(self.detect_lidar.closest_point_0[0] - self.detect_lidar.x_min_img2),
        #                     int(self.detect_lidar.closest_point_0[1] - self.detect_lidar.y_min_img2)),3,(0,0,255),-1)
        # if len(self.detect_lidar.closest_point_1) != 0:
        #     cv2.circle(img,(int(self.detect_lidar.closest_point_1[0] - self.detect_lidar.x_min_img2),
        #                     int(self.detect_lidar.closest_point_1[1] - self.detect_lidar.y_min_img2)),3,(0,255,0),-1)
        # if len(self.detect_lidar.closest_point_2) != 0:
        #     cv2.circle(img,(int(self.detect_lidar.closest_point_2[0] - self.detect_lidar.x_min_img2),
        #                     int(self.detect_lidar.closest_point_2[1] - self.detect_lidar.y_min_img2)),3,(255,0,0),-1)
        ...
        if self.detect_lidar.connect_driver == True:
            ...
            connect = False
        # if self.py_esp.connected == True:
        #     cv2.putText(img, "ESP32: connected", (400, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        # else:
        #     cv2.putText(img, "ESP32: disconnected", (400, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        #     connect = False
        # if self.scan_lidar.connect_lidar == True:
        #     cv2.putText(img, "Lidar:  connected", (400, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
        # else:
        #     cv2.putText(img, "Lidar:  disconnected", (400, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        #     connect = False
        # if self.ss_dk == 1:
        #     cv2.putText(img, "Load encoder: OK", (400, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        # else:
        #     cv2.putText(img, "Load encoder: NG", (400, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
        # self.detect_lidar.sent_done_to_plc(connect)
        ...
        # self.detect_lidar.angle_camera = self.py_esp.alpha_servo
        # if self.py_esp.connected == True:
        #     # print(str(int(self.detect_lidar.angle_controller)) + "\r\n")
        #     data_sent_esp = data_sent_esp +"#"+ str(int(self.detect_lidar.angle_controller)) + "\r\n"
        # self.py_esp.sent_data(data_sent_esp)
        
        cv2.imshow("img_Astar",self.detect_lidar.image_new)
        cv2.imshow("img_controller",self.detect_lidar.img_controller)
        cv2.imshow("map",img)
